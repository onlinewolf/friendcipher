...
//class for help
CWKData::CWKData(long blockLen):kBlockLen(blockLen){
    data_ = nullptr;
    pass_ = new uint8_t[kBlockLen];
    salt_ = new uint8_t[kBlockLen];
}
...
bool CryptWithKeccak::enCrypt(CWKData* data){
    if(!data || !data->data_ || !data->pass_ || !data->salt_ || data->kBlockLen != kBlockSize || !ivCreated_)
        return false;
...
bool CryptWithKeccak::deCrypt(CWKData* data){
    ...
    keyLen = 0;
    for(long i=0; i<data->dataLen_; keyLen++){
        blockLen = calcBlockSize(keys[keyLen], kBlockSize);
